...
\begin{frame}[fragile]
    ...
    amount using $m=90$ pseudo-bins and left-censored distribution.
...
\pause
    Climatological estimates for Limerick for today (July 17\textit{th}):
\smallskip
\begin{itemize}
    \item A $68.7\%$ probability of getting rain,
    \item a $44.2\%$ chance of receiving more than $1\,\text{mm}$,
    \item with an expected daily amount of $1.44\,\text{mm}$.
\end{itemize}
...
\begin{frame}[fragile]
    ...
    \textbf{Summary}
    \begin{itemize}
        \item Extended transition models to continuous response
        \item Performs en-par with e.g., ZANBI without assumptions on the distribution
        \item Allows to retrieve mean, median, mode, variance, quantiles, \dots
        \item Proven to work well with big data sets
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Install via: \url{https://retostauffer.r-universe.dev/builds}
        \item Toy-example in the proceeding
    \end{itemize}
