...
if (!all(file.exists(MFILES))) {
    ...
    library("ggplot2")
    p1 <- autoplot(r1,  main = tmp[1L], xlab = "x", fill = "gray85") +
        xlab(NULL) + theme(plot.margin = unit(c(.05, .1, 0, .2), "in"))
    p2 <- autoplot(r2,  main = tmp[2L], xlab = "x", fill = "gray85") +
        xlab(NULL) + theme(plot.margin = unit(c(.05, .1, 0, .2), "in"))
    p3 <- autoplot(r3,  main = tmp[3L], xlab = "x", fill = "gray85") +
        xlab(NULL) + theme(plot.margin = unit(c(.05, .1, 0, .2), "in"))
    p4 <- autoplot(rtr, main = tmp[4L], xlab = "x", fill = "gray85") +
        xlab(NULL) + theme(plot.margin = unit(c(.05, .1, 0, .2), "in"))
...
<<convert_cdf_to_pdf_simple, echo=FALSE, fig=TRUE, width=8, height=5.5>>=
...
<<convert_cdf_to_pdf_on_example, echo=FALSE, fig=TRUE, width=8, height=5.5>>=
# Doing it again just for the sake of completeness
load(RDAFILE)
h <- hist(y, breaks = bk, plot = FALSE)
cdf <- cumsum(h$density)[1:8]
b <- barplot(cdf, ylim = c(0, 1))
tp <- transitreg::convert_tp(cdf, from = "cdf", to = "tp")
...
text(b[1, 1], tp[1], sprintf("1 - %.2f = %.2f", cdf[1], tp[1]),
     srt = 90, adj = c(1.1, 0.5), col = "tomato", font = 2)
for (i in 2:length(b)) {
    tmp <- tp[i] * (1 - cdf[i])
    text(b[i, 1], tp[i], sprintf("%.2f (1 - %.2f) = %.2f", tp[i - 1], cdf[i], tp[i]),
         srt = 90, adj = c(1.1, 0.5), col = "tomato", font = 2)
...
grDevices::pdf(file = file.path("_images", "conditional-PO.pdf"), width = 6, height = 4)
    ...
    lines(q_x, p_PO[, 1], col = colors[1L], lwd = 3)
    lines(q_x, p_PO[, 2], col = colors[2L], lwd = 3)
    lines(q_x, p_PO[, 3], col = colors[1L], lwd = 3)
...
grDevices::pdf(file = file.path("_images", "conditional-NB.pdf"), width = 6, height = 4)
    par(mar = c(3.4, 3.3, .5, .5))
    plot(y ~ x, data = d, col = adjust_transparency(1, 0.1), xlab = NA, ylab = NA, cex = 0.6)
    lines(q_x, p_NB[, 1], col = colors[1L], lwd = 3)
    lines(q_x, p_NB[, 2], col = colors[2L], lwd = 3)
    lines(q_x, p_NB[, 3], col = colors[1L], lwd = 3)
...
grDevices::pdf(file = file.path("_images", "conditional-TR.pdf"), width = 6, height = 4)
    par(mar = c(3.4, 3.3, .5, .5))
    plot(y ~ x, data = d, col = adjust_transparency(1, 0.1), xlab = NA, ylab = NA, cex = 0.6)
    lines(q_x, p_TR[, 1], col = colors[1L], lwd = 3)
    lines(q_x, p_TR[, 2], col = colors[2L], lwd = 3)
    lines(q_x, p_TR[, 3], col = colors[1L], lwd = 3)
...
@
    ...
    \only<3>{
        Estimated model: ~~~ $y \sim \text{TP}(\theta_0 + \text{te}(theta, x, k = 10))$.\\[1ex]
