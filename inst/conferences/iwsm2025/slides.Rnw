...
\begin{frame}[fragile]
    ...
    \begin{itemize}
        ...
        \item Allow to account for excesses in any count(s)
        \item Be highly flexible in both it's parameters and the response distribution
        \item Allow to retrieve mean, median, mode, \dots
    \end{itemize}
...
\begin{frame}[fragile]
    ...
    \textbf{Simple example: marginal distribution}\\
    Let us assume a random count variable
    $Y \sim \text{NBI}^{\textcolor{uibkorange}{3\%}}_0(\text{size} = 4, \text{prob} = 0.3)$.

    \begin{figure}
        \center
<<simple_example, echo = FALSE, fig = TRUE, width = 7, height = 3>>=
#ggplot() + geom_histogram(aes(x = x), breaks = bk) + theme_minimal()
par(mar = c(2.5, 3, 1, 0))
h <- hist(y, breaks = bk, main = NULL, xlab = NULL, freq = FALSE)

rect(xleft = h$breaks[1], xright = h$breaks[2],
     ybottom = yzero, ytop = h$density[1], col = "#fdbe85")
text(h$mids[1], yzero, adj = c(-0.2, 0.35), srt = 90, "excess")
@
    \end{figure}
...
%% ## Color definition
%% colors <- c(poisson = "black", negbin = "red", zeroinf = "blue", transitreg = "steelblue")
%% 
%% ## Transitreg
...
<<marginal_rootogram_plot, echo = FALSE, include = FALSE>>=
library("topmodels")

# Expected marginal plots
MFILES <- sprintf("_images/marginal-%d.pdf", 1:4)
if (!all(file.exists(MFILES))) {
    ...
    IGN <- function(x, n) -logLik(x) / n
    tmp <- paste(c("Poisson", "negbin", "zero-infl negbin", "transitreg"),
                 sprintf("(IGN = %.3f)", sapply(list(m1, m2, m3, mtr), IGN, n = length(y))))

    library("ggplot2")
    p1 <- autoplot(r1,  main = tmp[1L], xlab = "x", fill = "gray85", expected_col = "steelblue") +
        xlab(NULL) + theme(plot.margin = unit(c(.05, .1, 0, .2), "in"))
    p2 <- autoplot(r2,  main = tmp[2L], xlab = "x", fill = "gray85", expected_col = "steelblue") +
        xlab(NULL) + theme(plot.margin = unit(c(.05, .1, 0, .2), "in"))
    p3 <- autoplot(r3,  main = tmp[3L], xlab = "x", fill = "gray85", expected_col = "steelblue") +
        xlab(NULL) + theme(plot.margin = unit(c(.05, .1, 0, .2), "in"))
    p4 <- autoplot(rtr, main = tmp[4L], xlab = "x", fill = "gray85", expected_col = 2) +
        xlab(NULL) + theme(plot.margin = unit(c(.05, .1, 0, .2), "in"))

    size <- list(w = 3.5, h = 1.5)
    ggsave(p1, file = MFILES[1L], width = size$w, height = size$h)
    ggsave(p2, file = MFILES[2L], width = size$w, height = size$h)
    ggsave(p3, file = MFILES[3L], width = size$w, height = size$h)
    ggsave(p4, file = MFILES[4L], width = size$w, height = size$h)
}
@
    ...
    \begin{figure}[width = \textwidth]
        \centering
        \begin{tabular}{cc}
        \visible<1->{\includegraphics[width=0.49\textwidth]{_images/marginal-1.pdf}} &
        \visible<2->{\includegraphics[width=0.49\textwidth]{_images/marginal-2.pdf}} \\
        \visible<3->{\includegraphics[width=0.49\textwidth]{_images/marginal-3.pdf}} &
        \visible<4->{\includegraphics[width=0.49\textwidth]{_images/marginal-4.pdf}} \\
        \end{tabular}
    \end{figure}

    \begin{footnotesize}
    Ignorance (IGN): $-\text{logLik}(\text{model}) / n$; the lower, the better.
    \end{footnotesize}
...
%%%%$$
%%$$
%%    \pi_{ir} = {\only<2->{\textcolor{steelblue}}{(1 - F(\eta_{ir}(\boldsymbol{\alpha})))}}
%%               {\only<2->{\textcolor{tomato}}{\prod_{s=0}^{r-1} F(\eta_{is}(\boldsymbol{\alpha}))}}.
%%$$
...
<<tp_table, echo = FALSE, results = hide>>=
tmp <- load(RDAFILE) # sicher ist sicher
h   <- hist(y, breaks = bk, plot = FALSE)
cdf <- cumsum(h$density)
pdf <- h$density
tp <- convert_tp(cdf, from = "cdf", to = "tp")
@

    \begin{table}
        \centering
        \begin{tabular}{c | c | l | c}
            $r$ & $F(\eta_{ir}(\alpha))$ & $\pi_{ir}$ & $\text{CDF}_{ir}$ \\
            \hline
            0 & \Sexpr{sprintf("%.3f", tp[1])} &
                \visible<2->{\textcolor{steelblue}{(1 - \Sexpr{sprintf("%.3f", tp[1])})} = \Sexpr{sprintf("%.3f", pdf[1])}} &
                \visible<2->{\Sexpr{sprintf("%.3f", cdf[1])}} \\
            1 & \Sexpr{sprintf("%.3f", tp[2])} &
                \visible<3->{\textcolor{steelblue}{(1 - \Sexpr{sprintf("%.3f", tp[2])})} $\cdot$ \textcolor{tomato}{\Sexpr{sprintf("%.3f", tp[1])}} = \Sexpr{sprintf("%.3f", pdf[2])}} &
                \visible<3->{\Sexpr{sprintf("%.3f", sum(cdf[1:2]))}} \\
            2 & \Sexpr{sprintf("%.3f", tp[3])} &
                \visible<4->{\textcolor{steelblue}{(1 - \Sexpr{sprintf("%.3f", tp[3])})} $\cdot$ \textcolor{tomato}{\Sexpr{sprintf("%.3f", tp[1])} $\cdot$ \Sexpr{sprintf("%.3f", tp[2])}}  = \Sexpr{sprintf("%.3f", pdf[3])}} &
                \visible<4->{\Sexpr{sprintf("%.3f", sum(cdf[1:3]))}} \\
            3 & \Sexpr{sprintf("%.3f", tp[4])} & \visible<4->{\dots} & \visible<4->{\dots} \\
        \end{tabular}
    \end{table}
...
\begin{center}
<<cdf_pdf_tp_plot, echo=FALSE, fig=TRUE, width=8, height=5>>=
par(mar = c(3.1, 3.1, 3, 0.5), xpd = TRUE)
py <- seq_along(tp) - 1
plot(NA, xlim = c(-0.35, length(tp) - 1 + 0.35), ylim = c(0, 1),
     yaxs = "i", xaxs = "i", ylab = NA, xlab = "y [counts]")

rect(py - 0.3, 0, py + 0.3, cdf, col = "gray80", border = NA)
rect(py - 0.3, 0, py + 0.3, pdf, col = "gray30", border = NA)
lines(py, tp, col = "tomato", type = "o", pch = 19, lwd = 2)
legend("top", ncol = 3, bty = "n",
       inset = c(0, -0.15), xpd = TRUE,
       fill   = c(NA, "gray80", "gray30"),
       border = c(NA, 1, 1),
       pch    = c(19, 0, 0),
       lty    = c(1, NA, NA),
       lwd    = c(2, NA, NA),
...
@
    ...
    \only<1>{
        Conditional model (simulated data).
        \begin{center}
...
\begin{frame}[fragile]
    ...
    \begin{itemize}
        ...
        \item Over 3.9\,million individual observations
    \end{itemize}
    ...
    amount using $m=90$ (non-uniform) pseudo-bins and left-censored distribution.
