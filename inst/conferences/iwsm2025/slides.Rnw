...
if (!file.exists(RDAFILE)) {
    ## Simulating data
    set.seed(1234)
    xnb <- rnbinom(5000, size = 4, prob = 0.3)
    x   <- c(rep(0L, 150), xnb)
    xzero <- 150 / length(x)
    bk <- seq.int(0L, max(x))
    ...
    save(x, xzero, bk,
         ...
         m3, m3_params, m3_dens, file = RDAFILE)
} else {
    load(RDAFILE)
}
@
...
\begin{frame}[fragile]
    ...
    $Y \sim \text{NB}^{3\%}_0(\text{size} = 4, \text{prob} = 0.3)$.
...
par(mar = c(2.5, 3, 1, 0))
h <- hist(x, breaks = bk, main = NULL, xlab = NULL, freq = FALSE)

rect(xleft = h$breaks[1], xright = h$breaks[2],
     ybottom = xzero, ytop = h$density[1], col = "lightblue")
text(h$mids[1], xzero, adj = c(-0.2, 0.35), srt = 90, "excess")
@
    \end{figure}
...
if (!all(file.exists(MFILES))) {
    ...
    tmp <- paste(c("Poisson", "negbin", "zero-infl negbin", "transitreg"),
                 sprintf("(IGN = %.3f)", sapply(list(m1, m2, m3, mtr), IGN, n = length(x))))
