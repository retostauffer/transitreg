...
%%  calculation %% %% ------------------------------------------------------------------
%%  calculation %% \begin{frame}[fragile]
%%  calculation %%     \frametitle{Methodology}
%%  calculation %% 
%%  calculation %% 
%%  calculation %% \begin{center}
%%  calculation %% <<cdf_pdf_tp_plot, echo=FALSE, fig=TRUE, width=8, height=5>>=
%%  calculation %% par(mar = c(3.1, 3.1, 3, 0.5), xpd = TRUE)
%%  calculation %% py <- seq_along(tp) - 1
%%  calculation %% plot(NA, xlim = c(-0.35, length(tp) - 1 + 0.35), ylim = c(0, 1),
%%  calculation %%      yaxs = "i", xaxs = "i", ylab = NA, xlab = "y [counts]")
%%  calculation %% 
%%  calculation %% rect(py - 0.3, 0, py + 0.3, cdf, col = "gray80", border = NA)
%%  calculation %% rect(py - 0.3, 0, py + 0.3, pdf, col = "gray30", border = NA)
%%  calculation %% lines(py, tp, col = "tomato", type = "o", pch = 19, lwd = 2)
%%  calculation %% legend("top", ncol = 3, bty = "n",
%%  calculation %%        inset = c(0, -0.15), xpd = TRUE,
%%  calculation %%        fill   = c(NA, "gray80", "gray30"),
%%  calculation %%        border = c(NA, 1, 1),
%%  calculation %%        pch    = c(19, 0, 0),
%%  calculation %%        lty    = c(1, NA, NA),
%%  calculation %%        lwd    = c(2, NA, NA),
%%  calculation %%        col    = c("tomato", NA, NA),
%%  calculation %%        legend = c("F()", "CDF", "PDF"))
%%  calculation %% @
%%  calculation %% \end{center}
%%  calculation %% 
%%  calculation %% \end{frame}
%%  calculation %% 
%%  calculation %% 
%%  calculation %% 
%%  calculation %% %% ------------------------------------------------------------------
...
%%  calculation %%     \frametitle{Methodology}
%%  calculation %% 
%%  calculation %% Parameter estimation considers the underlying Markov chain $Y_{i0}, Y_{i1}, \dots$,
%%  calculation %% where $Y_{ir} = \mathbf{1}(y_i = r)$. The log-likelihood simplifies to
%%  calculation %% $$
%%  calculation %% \ell(\boldsymbol{\alpha}) = \sum_{i=1}^n \sum_{s=0}^{y_i} \Big[Y_{is} \log(1 - F(\eta_{ir})) + (1 - Y_{is}) \log(F(\eta_{ir}))\Big].
%%  calculation %% $$
%%  calculation %% 
%%  calculation %% $(Y_{i0}, \dots, Y_{iy_i})^\top = (0, \dots, 0, 1)$ are created, along with a new
%%  calculation %% covariate $\theta_i = (0, 1, 2, \dots, y_i)^\top$ to capture
%%  calculation %% count-specific effects $f_j(\mathbf{x}_i, \theta_i)$, or simple count-specific
%%  calculation %% intercepts. All other covariates are duplicated accordingly.
%%  calculation %% 
%%  calculation %% \end{frame}
