...
\begin{frame}[fragile]
    ...
    into higher counts $r$ dependent on covariates $\mathbf{x}$ (Berger \& Tutz 2021).
$$
...
\begin{frame}[fragile]
    ...
    \begin{itemize}
        ...
        \item No need for prior assumptions about the response distribution
        \item Account for excesses in any count(s) (e.g., excess zeros)
        \item Highly flexible in both its parameters and the response distribution
        \item Allows retrieval of quantiles, mean, median, mode, \dots
    \end{itemize}
...
df <- subset(df, select = c(index, y, theta, Y, x))
@
...
\begin{array}{ll}
\tilde{y} \sim& \theta_0 + f(\theta) + f(\text{longitude},\text{latitude}) + f(\theta,\text{longitude},\text{latitude}) + \\
    & f(\text{day}) + f(\theta,\text{day}) + f(\text{altitude}) + f(\theta,\text{altitude}) + f(\text{year})
\end{array}
$$
    \end{footnotesize}
\pause
...
\smallskip
\begin{itemize}
    \item $69.7\%$ probability of getting rain,
    \item $46.0\%$ chance of more than $1\,\text{mm}$ ($0.4\%$ for $>31\,\text{mm}$),
    \item expected daily amount: $1.61\,\text{mm}$ (mode: $2.25\,\text{mm}$).
\end{itemize}
