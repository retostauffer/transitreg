...
\zeta_{l+1})$ for $l = 1, ..., m-1$ corresponds to a discrete pseudo-count $r$. Each observation $y_i$
...
$$
%%As the number of pseudo-bins increases ($m \rightarrow \infty$),  the
%%approximated PDF converges to the true PDF.
For a continuous response variable
$y_i$ with CDF $F(y)$, the probability of falling  into bin $[\zeta_l,
\zeta_{l+1})$ is approximated by  $P(\zeta_l \leq y_i < \zeta_{l+1}) =
F(\zeta_{l+1}) - F(\zeta_l)$.  These are encoded as transformed counts
...
$$
where the first term sums probabilities for bins below $\hat{y}_i$, and the
second term performs linear interpolation within the current bin.
...
weather station (Shannon airport, 20\,km west-northwest of Limerick) is estimated with a
generalized additive model for location, scale, and shape
employing a left-censored Gaussian distribution
(CN; estimated with \verb|bamlss|), quantile
regression (QR; using \verb|qgam|), and a left censored transition model (TM; with \verb|transitreg|, see Sec.\,\ref{stauffer:sec:software})
with fourty-one breaks ($m = 41$).
...
<<eval = FALSE>>=
install.packages("transitreg", repos = "https://retostauffer.r-universe.dev")
library("transitreg"); data("Shannon") # Loading package and data
