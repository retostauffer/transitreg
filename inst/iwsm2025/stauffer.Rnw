...
%%This discretization effectively provides a stepwise approximation of the
%%underlying smooth continuous distribution.
For a continuous response variable $y_i$ with CDF $F(y)$, the discretization
approximates the probabilities of $y_i$ falling into each interval as
$P(\zeta_l \leq y_i < \zeta_{l+1}) = F(\zeta_{l+1}) - F(\zeta_l)$.
...
weather station (Patrickswell, 8\,km southwest of Limerick) is estimated with a
...
\begin{figure}[h]
    ...
    \caption{Precipitation climatology for Patrickswell, Ireland, based on
