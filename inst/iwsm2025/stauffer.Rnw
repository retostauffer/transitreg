...
Rather then estimating the parameters of a specific distribution, transition
models instead model the probability of transitioning between intervals--that
is, the probability that a value exceeds a given threshold--while conditioning
...
\zeta_l+1)$ corresponds to a discrete pseudo-count $r$. Each observation $y_i$
...
All models use cyclic smooth splines on the day of the year as sole
covariate. The out-of-sample fit and pinball loss can be seen in
Figure\,\ref{stauffer:fig1}, showing that the transition model performs
comparably to the other two methods.
...
data from $1970$ to $2024$, from $557$ different weather stations across Ireland,
...
and interactions between these effects with $\theta$, resulting in the
...
\end{array}
$$
Given that the transition model represents the full (approximated) probabilistic
distribution, a wide range of quantities can be derived.
Figure\,\ref{stauffer:fig2} illustrates the estimates for July 16,
including the expected amount of square-root transformed daily precipitation (left),
the 99\textit{th} percentile (center), and the probability of observing more
than $1\,\sqrt{mm}$ of precipitation that day. The estimates indicate higher
...
areas that are associated with the hilliest regions in Ireland (cf. Walsh~2012).
...
\begin{figure}[h]
    ...
    Markers ($+$) denote stations used for model training.
    }
    \label{stauffer:fig2}
\end{figure}
...
Currently in alpha, a CRAN release is planned for Summer 2025.
