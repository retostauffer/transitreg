...
$$
P(y_i > r | y_i \ge r, \mathbf{x}_i) = F(\eta_{ir}(\alpha)),~~r = 0, 1, 2, \dots,
$$
...
%as the number of (pseudo-)bins increase ($m \rightarrow \infty$).
...
Besides the CDF, the transition model allows for computing additional key
...
Currently, the software is in beta and available via
GitHub (\texttt{https://github.com/retostauffer/transitreg}) as well as through the
\emph{R}-universe. A CRAN release is planned for Summer 2025.

The following code chunk estimates the left-censored transition model used in
Figure~\ref{stauffer:fig2} (center and left panels), without cross-validation.
