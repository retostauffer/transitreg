...
if(!file.exists("premodel.png") & FALSE) {
  ...
  set.seed(123)
  i <- sample(stations, size = floor(length(stations) * 0.6))

  dtrain <- subset(d, name %in% i)
  dtest <- subset(d, !(name %in% i))

  breaks <- c(-0.1, seq(0.1, floor(max(d$sqrt_pre)) + 1, by = 0.2))
  ...
  m1 <- tm(f1, data = dtrain, breaks = breaks, useC = TRUE)

  f2 <- Surv(sqrt_pre, sqrt_pre > 0) ~ s(day,bs="cc",k=20) + s(lon,lat) +
    te(day,lon,lat,d=c(1,2),bs=c("cr","tp"),k=c(5,30)) |
      s(day,bs="cc",k=20) + s(lon,lat) +
      te(day,lon,lat,d=c(1,2),bs=c("cr","tp"),k=c(5,30))
  ...
  AT <- read_sf("gadm41_AUT_1.json")
  Tyrol <- subset(AT, NAME_1 == "Tirol")
  Tyrol <- st_geometry(Tyrol)

  a <- st_intersects(dem, Tyrol)
}
@
