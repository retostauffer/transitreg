...
While transition models primarily estimate the cumulative distribution function (CDF) for 
distributional and quantile regression, they can also be used for modal regression, which 
identifies the most likely outcome (i.e., the mode) of a response variable. Modal regression is 
particularly useful for skewed, multimodal, or non-Gaussian data.
In this framework, the mode of the estimated probability density function (PDF) corresponds to 
the most probable value in the stepwise approximation. Given the estimated discrete 
probabilities,
$$
...
$$
Alternatively, a smoothed mode estimate can be obtained by interpolating between bin midpoints 
and refining the bin discretization for greater accuracy.
