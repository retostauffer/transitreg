...
if(!file.exists("ts_hist.png")) {
  ...
  j <- order(dtest$day)
  matplot(dtest$day[j], pb2[j, 3], type = "l", lty = 1, col = 2, add = TRUE)
  matplot(dtest$day[j], pg2[j, 3], type = "l", lty = 1, col = 5, add = TRUE)
  matplot(dtest$day[j], pm2[j, 3], type = "l", lty = 1, col = 4, add = TRUE)

  legend("topleft", paste(c("CN 99%-Q Err. =", "QR 99%-Q Err. =", "TM 99%-Q Err. ="),
