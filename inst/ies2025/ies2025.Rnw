...
if(!file.exists("ts_hist.png")) {
  ...
  legend("topleft", paste(c("CN pinLoss =", "QR pinLoss =", "TM pinLoss ="),
...
\caption{\label{fig:data} Precipitation data for Kirchberg in Tirol in Austria.
  The left panel presents a histogram of the square root-transformed precipitation values,
  overlaid with the fitted densities of a censored normal distribution (CN, red line)
  and a transition model (TM, blue line).
  The right panel illustrates the seasonal variation in square root-transformed 
  precipitation values, with empirical quantiles (1st, 10th, 50th, 90th, and 99th 
  percentiles) plotted for each day of the year. Estimates are shown for CN, TM and
  a quantile regression model (QR) together with the respective pin loss.}
\end{figure}
