...
if(!file.exists("ts_hist.png")) {
  ...
  ## TODO(R): Da geht was schief mit der normalen predict methode
  dm <- as.vector(predict(m, type = "pdf", y = mids)[1, ])
  library("topmodels")
  dm <- pdf(m[1], mids)
  ...
  pm2 <- predict(m2, newdata = dtest, prob = qu, type = "quantile")
  ...
  err_m <- sum(err_m)
  err_g <- sum(err_g)
  print(c(err_bamlss = err_b, err_transitreg = err_m, err_qgam = err_g))
