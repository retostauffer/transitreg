...
if(!file.exists("ts_hist.png")) {
  ...
  b <- gamlss2(Surv(sqrt_pre, sqrt_pre > 0) ~ 1, data = df, family = NOlc)
  par <- predict(b, newdata = data.frame("sqrt_pre" = mids))
  db <- family(b)$d(Surv(mids, mids > 0), par)
  ...
  f <- Surv(sqrt_pre, sqrt_pre > 0) ~ s(day,k=20,bs="cc") | s(day,k=20,bs="cc")
  b2 <- gamlss2(f, data = dtrain, family = NOlc)
  ...
  matplot(dtest$day[j], pg2[j, ], type = "l", lty = 1, col = 5, add = TRUE)
  matplot(dtest$day[j], pm2[j, ], type = "l", lty = 1, col = 4, add = TRUE)

  legend("topleft", paste(c("CN PBL =", "QR PBL =", "TM PBL ="),
      round(c(err_b, err_g, err_m))),
    lwd = 2, col = c(2, 5, 4), bty = "n")
...
if(!file.exists("premodel.png") & FALSE) {
  ...
  f1 <- sqrt_pre ~ theta0 + s(theta,k=20) +
    s(day,bs="cc",k=20) +
    te(theta,day,lon,lat,d=c(1,1,2),k=c(5,5,30)) +
    te(theta,day,bs=c("cr", "cc")) +
    te(theta,alt) +
    te(theta,lon,lat,d=c(1,2),bs=c("cr","tp"))

  m1 <- tm(f1, data = dtrain, breaks = breaks)

  f2 <- Surv(sqrt_pre, sqrt_pre > 0) ~ s(day,bs="cc",k=20) + s(alt) + s(lon,lat) +
    te(day,lon,lat,d=c(1,2),bs=c("cr","tp"),k=c(5,30)) |
      s(day,bs="cc",k=20) + s(alt) + s(lon,lat) +
      te(day,lon,lat,d=c(1,2),bs=c("cr","tp"),k=c(5,30))

  gen.cens("NO", type = "left")

  m2 <- gamlss2(f2, data = dtrain, family = NOlc, binning = TRUE)

  qu <- c(0.01, 0.1, 0.5, 0.9, 0.99)
  err1 <- err2 <- NULL
  par <- predict(m2, newdata = dtest)
  for(j in qu) {
  ...
  nd$m1_p99 <- predict(m1, newdata = nd, prob = 0.99)

  nr <- raster(dem)
  values(nr) <- nd$m1_p99

  plot(nr, main = "Predicted 99th Percentile (m1_p99)")

  par <- predict(m2, newdata = nd)
  nd$m2_p99 <- NA
  nd$m2_p99[!is.na(nd$alt)] <- family(m2)$q(0.99, par)
  nr2 <- raster(dem)
  values(nr2) <- nd$m2_p99

  plot(nr2, main = "Predicted 99th Percentile (m1_p99)")
}
@
%% \includegraphics[width=0.9\textwidth]{premodel.png}
